...
def getCustomScreenTemplate(String entityTypeName, String fieldTypeValue) {
    ...
    //String parentTypeValue = from(entityTypeName).where("invoiceTypeId", fieldTypeValue).cache().queryFirst().parentTypeId
    String parentTypeValue = from(entityTypeName).where(getFieldTypeName(entityTypeName), fieldTypeValue).cache().queryFirst().parentTypeId
