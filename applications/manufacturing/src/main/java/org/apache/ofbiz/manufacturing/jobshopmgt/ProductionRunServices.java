...
public class ProductionRunServices {
    ...
    private static void updateWorkOrderItemFulfillment(DispatchContext ctx,
                                                       String requirementId, String requirementStartDate,
                                                       String productionRunId, String productId)
        ...
        GenericValue mrpEvent = EntityQuery.use(delegator).from("MrpEvent").where(
                "productId", productId,
                "eventName", "*" + requirementId + " (" + requirementStartDate + ")*",
                "mrpEventTypeId", "PROP_MANUF_O_RECP"
        ).queryOne();

        if (mrpEvent == null)
            return;

        String mrpId = mrpEvent.getString("mrpId");
        mrpEvent = EntityQuery.use(delegator).from("MrpEvent").where(
                "mrpId", mrpId,
                "productId", productId,
        ...
        String eventName = mrpEvent.getString("eventName");
        if (eventName == null)
            return;
        ...
        String orderId = eventName.substring(0, index);
        String orderItemSeqId = eventName.substring(index + 1);
    ...
    public static Map<String, Object> createProductionRunFromRequirement(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        try {
            updateWorkOrderItemFulfillment(ctx,
                requirementId, "" + requirement.getTimestamp("requirementStartDate"),
                productionRunId, requirement.getString("productId"));
        } catch (GenericEntityException e) {
            String errMsg = "Problem calling the updateWorkOrderItemFulfillment()";
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
