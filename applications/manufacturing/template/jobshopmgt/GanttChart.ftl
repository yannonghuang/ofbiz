...
<#list phaseTaskList as t>
    ...
    <#if "PROD_ORDER_HEADER" == t.workEffortTypeId>
        g.AddTaskItem(new JSGantt.TaskItem("${t.workEffortId}", "${t.sequenceNum!}. ${t.workEffortName}", "", "", "00ff00", "${t.url}", 0, "", 0, 1, 0, 1<#if t.predecessor??>, "${t.predecessor}"</#if>));
    </#if>
    <#if "PROD_ORDER_TASK" == t.workEffortTypeId>
        g.AddTaskItem(new JSGantt.TaskItem("${t.workEffortId}","${t.sequenceNum!}. ${t.workEffortName}", "${StringUtil.wrapString(t.estimatedStartDate)}", "${StringUtil.wrapString(t.estimatedCompletionDate)}", "009900", "${t.url}", 0 , "", 0 , 0, ${t.workEffortParentId}, 1<#if t.predecessor??>, "${t.predecessor}"</#if>));
    </#if>
