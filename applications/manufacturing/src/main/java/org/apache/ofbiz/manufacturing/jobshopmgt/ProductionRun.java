...
public class ProductionRun {
        ...
        try {
            if (!UtilValidate.isEmpty(productionRunId)) {
                ...
                if (exist()) {
                    ...
                    this.setProductionRunComponents(delegator);
                }
            }
    ...
    private void setRequirement(Delegator delegator) throws GenericEntityException {
        if (productionRunComponents == null) return;

        for (GenericValue component : productionRunComponents) {
            component.set("statusId", ""); // use "statusId" field to hold "requirementId"

            GenericValue mrpEvent = EntityQuery.use(delegator).from("MrpEvent").where(
                    "productId", component.get("productId"),
                    "mrpEventTypeId", "PROP_PUR_O_RECP").queryOne();

            if (mrpEvent == null) continue;
            ...
            String requirementId = eventName.substring(0, index).trim();

            index = requirementId.indexOf("*");
            requirementId = requirementId.substring(index + 1).trim();

            component.set("statusId", requirementId); // should be "requirementId"
        }
    }

    private void setProductionRunComponents(Delegator delegator) throws GenericEntityException {
        if (exist()) {
            if (productionRunComponents == null) {
